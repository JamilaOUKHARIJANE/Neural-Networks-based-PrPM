...
def create_prefix_suffixes(df, window_size, outcome, case_id = 'case:concept:name', 
                           timestamp = 'time:timestamp', act_label = 'concept:name', res_label= 'org:resource',
    ...
    prefix_cols = [case_id, act_label,res_label, timestamp, 'ts_start', 'ts_prev', 'case_length'] + cat_casefts + num_casefts + cat_eventfts + num_eventfts
    prefix_subset = df[prefix_cols].copy()

    # Selecting needed columns for decoder suffixes 
    # NOTE: actually timestamp not needed anymore 
    suffix_cols = [case_id, act_label,res_label, timestamp, 'ts_start', 'ts_prev', 'case_length']
    ...
    # Selecting needed columns for resource label suffixes
    resLabel_cols = [case_id, res_label, 'case_length']
    resLabel_subset = df[resLabel_cols].reset_index(drop=True).copy()
    resLabel_subset['evt_idx'] = resLabel_subset.groupby(['case:concept:name']).cumcount() + 1
    ...
    #resource
    end_res_list = ['END_TOKEN' for _ in range(len(case_length_list))]
    ...
    # DF already sorted properly. Adding int case id to be able to sort on it again
    # further down the line for adding END resource.
    resLabel_subset = resLabel_subset.merge(case_data, on=case_id, how='left')
    end_dict = {case_id: case_id_list,
                res_label: end_res_list,
                'case_length': case_length_list,
                'evt_idx': evt_idx_list,
                'case_id_int': case_id_int}
    end_df = pd.DataFrame(end_dict)
    resLabel_subset = pd.concat([resLabel_subset, end_df], axis=0)
    # Making sure that each case's end event is concatenated after its last event.
    resLabel_subset = resLabel_subset.sort_values(['case_id_int', 'evt_idx'], ascending=[True, True], kind='mergesort')
    resLabel_subset = resLabel_subset.drop('evt_idx', axis=1).reset_index(drop=True)
    ...
    # res labels (shifted)
    resLabel_df = resLabel_subset[resLabel_subset['case_length'] >= 1].groupby(case_id, sort=False).tail(-1)
    resLabel_df['prefix_nr'] = 1
    resLabel_df['orig_case_id'] = resLabel_df[case_id].copy()
    ...
    for nr_events in tqdm(range(2, window_size+1)):
        ...
        # res label suffixes:
        tmp_reslab = resLabel_subset[resLabel_subset['case_length'] >= nr_events].groupby(case_id, sort = False).tail(-nr_events)
        tmp_reslab['orig_case_id'] = tmp_reslab[case_id].copy()
        tmp_reslab[case_id] = tmp_reslab[case_id] + '_{}'.format(nr_events)
        tmp_reslab['prefix_nr'] = nr_events
        resLabel_df = pd.concat([resLabel_df, tmp_reslab], axis = 0)
    ...
    resLabel_df.drop(['case_id_int'], axis = 1, inplace = True)
    actLabel_df.drop(['case_id_int'], axis=1, inplace=True)
    
    if outcome:
        return prefix_df, suffix_df, timeLabel_df, actLabel_df, outcomeLabel_df, resLabel_df
    else:
        return prefix_df, suffix_df, timeLabel_df, actLabel_df, resLabel_df
