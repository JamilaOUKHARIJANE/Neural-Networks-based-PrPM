...
def generate_timeLabel_tensor(case_id, 
    ...
    num_tens = torch.full((num_prefs, window_size, num_nums), -100.0, dtype=torch.float32)
...
def generate_resLabel_tensors(case_id,
                              res_label,
                              resLabel_df,
                              cardinalities,
                              window_size,
    ...
    Returns
    -------
    res_lab_tens = tf.Tensor
        resource labels. Tensor of shape (num_prefs, window_size) and of
        dtype tf.int32. The integer encodings that originally started from
        0 are now shifted by 1. 0 becomes the label indicating the padding
        token.
        Note that the cardinality of these resources, compared to
        cardinality 'car' of resources in prefixes and decoder suffixes,
        is 'car'+2. This is due to the fact that here we have to account for
        2 additional resource tokens that we did not have to acccount for
        previously: END_TOKEN (idx 'car'+1) and padding token (idx 0).
    """

    # Cardinality + 1 here bcs for the activity labels, we have an
    # additional END TOKEN that needs to be predicted!
    car = cardinalities[res_label] + 1
    # The current act_label column is still encoded as a categorical
    resLabel_df[res_label] = resLabel_df[res_label].astype(str)
    cat_res_labels = [str(i) for i in range(car - 1)] + ['END_TOKEN']
    int_res_labels = [i for i in range(1, car + 1)]
    str_labels_to_int = dict(zip(cat_res_labels, int_res_labels))
    resLabel_df[res_label] = resLabel_df[res_label].map(str_labels_to_int)
    # Computing the event idx for each unique prefix.
    resLabel_df['evt_idx'] = resLabel_df.groupby([case_id], sort=False).cumcount()

    resLabel_df['prefix_id_int'] = resLabel_df[case_id].map(str_to_int)

    # Computing the indices for the tensor scatter updates
    idx = torch.from_numpy(resLabel_df[['prefix_id_int', 'evt_idx']].to_numpy())  # (num_rows, 2)
    num_prefs = max(resLabel_df['prefix_id_int']) + 1  # number of distinct prefixes

    res_updates = torch.from_numpy(resLabel_df[res_label].to_numpy())  # shape (num_rows, )
    res_updates = res_updates.type(torch.int64)
    # Initializing resource label tensor by filling it with 0s.
    res_lab_tens = torch.zeros(size=[num_prefs, window_size], dtype=torch.long)  # (num_prefs, window_size)
    # Filling for each of the prefix-suffix pairs the resource label indices that correspond resual labels.
    # Therefore, 0th output label corresponds to the padding label.
    res_lab_tens[idx[:, 0], idx[:, 1]] = res_updates  # (num_prefs, window_size)
...
def generate_tensordata(pref_suff, case_id, act_label, cardinality_dict,res_label,
                        num_cols_dict,
                        cat_cols_dict, window_size, outcome):
    ...
    resLabel_df = pref_suff[4]
    if outcome:
        outcomeLabel_df = pref_suff[4]
        resLabel_df = pref_suff[5]
    ...
    timeLabel_tens = generate_timeLabel_tensor(case_id, timeLabel_df, 
                                               num_cols = num_cols_dict['timeLabel_df'], 
                                               window_size = window_size, 
                                               str_to_int=str_to_int)
    ...
    print("Generating resource label tensors ...")
    resLabel_tens = generate_resLabel_tensors(case_id, res_label, resLabel_df, cardinalities=cardinality_dict,
                                              window_size=window_size,
                                              str_to_int=str_to_int)
    
    # Make one big tuple containing all the tensors 
    all_tensors = prefix_tensors + suffix_tensors + timeLabel_tens + (actLabel_tens,) + (resLabel_tens,)
...
def generate_tensordata_train_test(train_pref_suff, 
    ...
    train_data = generate_tensordata(pref_suff = train_pref_suff, case_id = case_id, 
                                     act_label = act_label, res_label = res_label,
                                     cardinality_dict = cardinality_dict,
