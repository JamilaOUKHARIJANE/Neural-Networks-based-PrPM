...
def remove_overlapping_trainPrefs(train_pref_suff, 
    ...
    resLabel_df_train = train_pref_suff[4]
    if outcome:
        outcomeLabel_df_train = train_pref_suff[4]
        resLabel_df_train = train_pref_suff[5]
    ...
    if len(prefix_df_overlap)>0:
        ...
        resLabel_df_train = resLabel_df_train[~resLabel_df_train[case_id].isin(invalid_prefix_ids)].copy()
        if outcome:
            outcomeLabel_df_train = outcomeLabel_df_train[~outcomeLabel_df_train[case_id].isin(invalid_prefix_ids)].copy()
    if outcome:
        return prefix_df_train, suffix_df_train, timeLabel_df_train, actLabel_df_train, outcomeLabel_df_train, resLabel_df_train
    else: 
        return prefix_df_train, suffix_df_train, timeLabel_df_train, actLabel_df_train, resLabel_df_train
...
def remove_overlapping_testPrefs(test_pref_suff, 
    ...
    resLabel_df_test = test_pref_suff[4]
    if outcome:
        outcomeLabel_df_test = test_pref_suff[4]
        resLabel_df_test = test_pref_suff[5]
    ...
    resLabel_df_test = resLabel_df_test[~resLabel_df_test[case_id].isin(invalid_prefix_ids)]
    if outcome:
        outcomeLabel_df_test = outcomeLabel_df_test[~outcomeLabel_df_test[case_id].isin(invalid_prefix_ids)]
        resLabel_df_test = resLabel_df_test[~resLabel_df_test[case_id].isin(invalid_prefix_ids)]

        return prefix_df_test, suffix_df_test, timeLabel_df_test, actLabel_df_test, outcomeLabel_df_test, resLabel_df_test
    else:
        return prefix_df_test, suffix_df_test, timeLabel_df_test, actLabel_df_test,resLabel_df_test
...
def main_dataframe_pipeline(log,
    ...
    """
    if mode not in ['preferred', 'workaround', 'simple']:
    ...
    train_df = create_numeric_timeCols(train_df, case_id, timestamp, act_label, res_label)
    test_df = create_numeric_timeCols(test_df, case_id, timestamp, act_label, res_label)
    val_df = create_numeric_timeCols(val_df, case_id, timestamp, act_label, res_label)
    ...
    pref_catcols = cat_casefts + cat_eventfts + [act_label] + [res_label]
    suff_catcols = [act_label, res_label]
    actlab_catcols = [act_label]
    reslab_catcols = [res_label]
    ...
    cat_cols_dict = {'prefix_df': pref_catcols, 'suffix_df': suff_catcols, 'actLabel_df': actlab_catcols, 'resLabel_df': reslab_catcols}
