...
def trainTestSplit(df,file_name, test_len, case_id, timestamp, mode):
    ...
    if mode=='preferred':
        ...
        act_name_key = 'concept:name'  # 'Activity code'
        res_name_key = 'org:resource'  # 'org:resource' #
        timestamp_key = 'time:timestamp'

        train_log = pm4py.read_xes(f"logs/{file_name}_train.xes")
        #[                [case_name_key, act_name_key, res_name_key, timestamp_key]]
        test_log = pm4py.read_xes(f"logs/{file_name}_test.xes")
        #[            [case_name_key, act_name_key, res_name_key, timestamp_key]]

        train_ids = train_log[case_id].unique().tolist()
        test_ids = test_log[case_id].unique().tolist()
        df_train = df[df[case_id].isin(train_ids)].copy().reset_index(drop=True)
        df_test = df[df[case_id].isin(test_ids)].copy().reset_index(drop=True)

        # List of ids of all training cases that start before, but end after, separation time
        train_case_ids_overlap = list(set(train_ids) - set(test_ids))
        # Retaining only the training traces ending after separation time
        df_train_overlap = df_train[df_train[case_id].isin(train_case_ids_overlap)].copy().reset_index(drop=True)
        # Adding zero-based event indices for each case
        df_train_overlap['evt_idx'] = df_train_overlap.groupby([case_id]).cumcount()
        # Slicing out only those events that occur before separation time
        df_train_overlap_prefixes = df_train_overlap[
            df_train_overlap[timestamp].values < first_test_start_time].copy().reset_index(drop=True)
        # Slicing out only the last event that occurred before separation
        # time for each case
        df_train_overlap_prefixes = df_train_overlap_prefixes.groupby(case_id, sort=False,
...
def remainTimeOrClassifBenchmark(dataset, 
    ...
    dataset_short =dataset #limited_duration(dataset, max_days, case_id, timestamp)
    


    # Split dataset in train and test set, applying strict temporal splitting and debiasing the test set
    dataset_train, dataset_test, prefix_dict = trainTestSplit(dataset_short, file_name= file_name,
