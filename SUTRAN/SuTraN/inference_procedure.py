...
def inference_loop(model, 
    ...
    * `num_activities` : the total number of possible activity labels 
    ...
    with torch.no_grad():
        ...
        suffix_res_decoded_global = torch.empty((0, window_size), dtype=torch.int64)
        ...
        res_labels_global = inference_dataset[res_label_index]  # (num_prefs, window_size)
        num_resources = torch.max(res_labels_global).item() + 1
        ...
        num_prefs_res = res_labels_global.shape[0]
        ...
        suf_len_global_res = torch.argmax((res_labels_global == (num_resources - 1)).to(torch.int64),
                                      dim=-1) + 1  # (num_prefs,)
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                ...
                #   - Greedily decoded resource suffix
                suffix_res_decoded = voutputs[-1] # (B, W) torch.int64
                suffix_res_decoded_global = torch.cat((suffix_res_decoded_global, suffix_res_decoded.cpu()), dim=0)
                ...
                if only_rrt: 
                    ...
                    out_pred = voutputs[-2] # (B, ) torch.float32
                    out_pred_global = torch.cat((out_pred_global, out_pred.cpu()), dim=-1)
                
                elif both:
                    # - (direct) remaining runtime predictions. 
                    #   Still in standardized scale. 
                    rrt_pred = voutputs[-3] # (B,) torch.float32
                    rrt_pred_global = torch.cat((rrt_pred_global, rrt_pred.cpu()), dim=-1)
        ...
        outputs_global = (suffix_acts_decoded_global,suffix_res_decoded_global, suffix_ttne_preds_global)
        ...
        if results_path: 
            ...
            #   Resource suffix predictions
            suffix_res_decoded_path = os.path.join(subfolder_path, 'suffix_res_decoded.pt')
            torch.save(suffix_res_decoded_global, suffix_res_decoded_path)
        ...
        dam_lev, dam_lev_res = infer_env.damerau_levenshtein_distance_tensors() # (num_prefs, )
        ...
        dam_lev_res_similarity = 1. - dam_lev_res  # (num_prefs,)
        avg_dam_lev_res = (torch.sum(dam_lev_res_similarity) / dam_lev_res_similarity.shape[0]).item()  # Scalar
        ...
        if results_path:
            ...
            dam_lev_res_sim_path = os.path.join(subfolder_path, 'dam_lev_res_similarity.pt')
            torch.save(dam_lev_res_similarity, dam_lev_res_sim_path)
    ...
    return_list += [avg_dam_lev, avg_dam_lev_res, percentage_too_early, percentage_too_late]
    ...
    for i in range(1, window_size+1):
        ...
        dam_levs_res = dam_lev_res_similarity[bool_idx].clone()
        ...
        if num_inst > 0:
            ...
            avg_dl_res = (torch.sum(dam_levs_res) / num_inst).item()
            avg_mae = (torch.sum(MAE_rrt_i) / num_inst).item()
            results_i = [avg_dl, avg_dl_res, avg_mae, num_inst]
