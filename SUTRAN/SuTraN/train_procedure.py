...
def train_epoch(model, 
    ...
    running_loss_res = []  # Cross-Entropy
    ...
    for batch_num, data in tqdm(enumerate(training_loader), desc="Batch calculation at epoch {}.".format(epoch_number)):
        ...
        running_loss_res.append(loss_results[2])
        running_loss_ttne.append(loss_results[3])
    ...
    last_running_avg_res = sum(running_loss_res[-batch_interval:]) / batch_interval
    print(
        "Running average resource prediction loss: {} (Cross Entropy over last {} batches)".format(last_running_avg_res,
                                                                                                   batch_interval))
    ...
    if only_rrt:
        ...
        last_running_avgs = last_running_avg_glob, last_running_avg_act,last_running_avg_res, last_running_avg_ttne, last_running_avg_rrt, loss
...
def train_model(model, 
    ...
    * `outcome_bool` : For the paper implementation, this boolean should 
    ...
    avg_dam_lev_glob,avg_dam_lev_res_glob, perc_too_early_glob, perc_too_late_glob, perc_correct_glob = ([] for _ in range(5))
    ...
    loss_fn = MultiOutputLoss(num_activities,  num_resources, remaining_runtime_head, outcome_bool)
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        train_losses_res.append(last_running_avgs[2])
        train_losses_ttne.append(last_running_avgs[3])
        last_loss = last_running_avgs[-1]
        if only_rrt:
            train_losses_rrt.append(last_running_avgs[4])
        elif only_out:
        ...
        if only_rrt:
            ...
            avg_MAE_stand_RRT = inf_results[10]
            # MAE RRT converted to minutes
            avg_MAE_minutes_RRT = inf_results[11]
        
        elif only_out:
            # Binary Cross Entropy outcome prediction
            avg_BCE_out = inf_results[10]
            # AUC-ROC outcome prediction
            auc_roc = inf_results[11]
            # AUC-PR outcome prediction
            auc_pr = inf_results[12]
            ...
            avg_MAE_minutes_RRT = inf_results[11]
            # Binary Cross Entropy outcome prediction
            avg_BCE_out = inf_results[12]
            # AUC-ROC outcome prediction
            auc_roc = inf_results[13]
            # AUC-PR outcome prediction
        ...
        if avg_dam_lev_res > best_DL_sim_res:
            better = True
            best_DL_sim_res = avg_dam_lev_res
        ...
        avg_dam_lev_res_glob.append(avg_dam_lev_res)
    ...
    if both:
        results = pd.DataFrame(data = {'epoch' : epoch_list, 
                            ...
                            'Resource suffix: 1-DL (validation)': avg_dam_lev_res_glob,
        ...
        results = pd.DataFrame(data = {'epoch' : epoch_list, 
                            ...
                            'TTNE - minutes MAE validation': avg_MAE_ttne_minutes_glob, 
                            'Activity suffix: 1-DL (validation)': avg_dam_lev_glob,
                            'Activity suffix: 1-DL (validation)': avg_dam_lev_res_glob,
                            'Percentage too early (validation)': perc_too_early_glob,    
                            'Percentage correct END prediction (validation)': perc_correct_glob,   
                            'Percentage too late (validation)': perc_too_late_glob,   
                            'Avg absolute amount of events predicted too early / too late (validation)': mean_absolute_length_diff_glob, 
