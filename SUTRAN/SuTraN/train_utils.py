...
class MultiOutputLoss_1(nn.Module):
    def __init__(self, num_classes, num_resources):
        ...
        self.cat_loss_fn_res = MaskedCrossEntropyLoss(num_resources)
    ...
    def forward(self, outputs, labels):
        ...
        # Loss resource suffix prediction
        cat_loss_res = self.cat_loss_fn_res(outputs[-1], labels[-1])
        ...
        loss = cat_loss + cat_loss_res + cont_loss1 + cont_loss2

        # Composite loss, act suffix loss, ttne loss, rrt loss
        return loss, cat_loss.item(), cat_loss_res.item(), cont_loss1.item(), cont_loss2.item()
...
class MultiOutputLoss(nn.Module):
    def __init__(self, num_classes,num_resources, remaining_runtime_head, outcome_bool):
        ...
        if only_rrt:
            self.composite_loss = MultiOutputLoss_1(num_classes=num_classes, num_resources = num_resources)
