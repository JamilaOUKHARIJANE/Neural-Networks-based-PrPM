...
class BatchInference():
    def __init__(self,
        ...
        * `num_activities` : the total number of possible activity labels 
        ...
        if only_rrt: 
            ...
            self.res_labels = labels[-1]  # (num_prefs, window_size)
        ...
        self.num_resources = torch.max(self.res_labels).item() + 1
        ...
        self.pred_length_res = self.edit_suffix_res()
        self.actual_length = self.get_actual_length()
        self.actual_res_length = self.get_actual_res_length()
    ...
    def edit_suffix_res(self):
        ...
        # Initialize tensor that contains 1 where end token
        # (idx num_classes-1) is predicted, 0 otherwise.
        end_pred_tens = (self.suffix_res_decoded== (self.num_resources-1)).to(torch.int64) # (batch_size, window_size)
        # Init tensor that contains for each instance the number of times
        # an end token is predicted
        sum_tens = torch.sum(input=end_pred_tens, dim=-1) # (batch_size,)
        ...
        # Artificial end token inserted on the last time step, for all
        # instances for which the model never produced an END prediction
        suffix_res_dec_help = self.suffix_res_decoded.clone() # (batch_size, window_size)
        suffix_res_dec_help[:,-1] = self.num_resources-1
        # (batch_size, window_size)
        self.suffix_res_decoded = torch.where(condition=(sum_tens==0).unsqueeze(-1), input=suffix_res_dec_help, other=self.suffix_res_decoded)

        # Right padding decoded resource predictions with index 0
        # starting from the first END token prediction
        pred_length_res = torch.argmax((self.suffix_res_decoded== (self.num_resources-1)).to(torch.int64), dim=-1)
        counting_tensor = torch.arange(self.window_size, dtype=torch.int64).to(device) # (window_size,)
        #       Repeat the tensor along the first dimension to match the desired shape
        counting_tensor = counting_tensor.unsqueeze(0).repeat(self.batch_size, 1).to(device) # (batch_size, window_size)
        padding_bool = counting_tensor > pred_length_res.unsqueeze(-1) # (batch_size, window_size)
        padding_inds = torch.nonzero(input=padding_bool, as_tuple=True)
        self.suffix_res_decoded[padding_inds] = 0

        return pred_length_res
    ...
    def get_actual_res_length(self):
        """Get the ground truth suffix length of each of the batch_size
        instances. Computed based on the act_labels, by determining
        the suffix index where the ground truth 'END TOKEN' is located.
        Note: this is 0-based. E.g., a value of 0 would indicate that the
        ...
        decoding step.
        """
        actual_length = torch.argmax((self.res_labels == (self.num_resources-1)).to(torch.int64), dim=-1) # (batch_size,)
        return actual_length
    ...
    def damerau_levenshtein_distance_tensors(self):
        ...
        * `self.actual_length` : torch.Tensor of dtype torch.int64 and 
        ...
        len_pred_res = self.pred_length_res + 1  # (self.batch_size, )
        len_actual_res = self.actual_res_length + 1  # (self.batch_size, )
        ...
        max_len_res = torch.maximum(len_pred_res, len_actual_res)
        ...
        d_res = torch.full(size=(self.batch_size, self.window_size + 1, self.window_size + 1), fill_value=0,
                       dtype=torch.int64).to(device)  # (B, WS+1, WS+1)
        ...
        for i in range(1, self.window_size+1): 
            ...
            for j in range(1, self.window_size+1):
                ...
                d_res[:, i, j] = torch.minimum(torch.minimum(deletion_res, insertion_res), substition_res)
                ...
                if i > 1 and j > 1:
                    ...
                    tpos_true_res = (self.suffix_res_decoded[:, i - 1] == self.res_labels[:, j - 2]) & (
                                self.suffix_res_decoded[:, i - 2] == self.res_labels[:, j - 1])
                    ...
                    min_og_tpos_res = torch.minimum(d_res[:, i, j], d_res[:, i - 2, j - 2] + cost_res)  # (self.batch_size, )
                    ...
                    d_res[:, i, j] = torch.where(tpos_true_res, min_og_tpos_res, d_res[:, i, j])
        ...
        batch_arange_res = torch.arange(start=0, end=self.batch_size, dtype=torch.int64).to(device)
        ...
        dam_lev_dist_res = d_res[batch_arange_res, len_pred_res, len_actual_res] / max_len_res
        return dam_lev_dist, dam_lev_dist_res
