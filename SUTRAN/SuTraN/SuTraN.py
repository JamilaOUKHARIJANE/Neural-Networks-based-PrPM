...
class SuTraN(nn.Module):
    ...
    def forward(self, 
        ...
        # Concatenate cat_emb with the numerical fe,atures to get initial vector representations prefix events.
...
class SuTraN_no_context(nn.Module):
    def __init__(self, 
        ...
        * `outcome_bool` : For the paper implementation, this boolean should 
        ...
        # Shared embedding size calculation for resources
        self.resource_emb_size = min(600, round(1.6 * (num_resources - 2) ** 0.56))
        self.res_emb = nn.Embedding(
            num_embeddings=num_resources - 1,
            embedding_dim=self.resource_emb_size,
            padding_idx=0
        )

        # Combined embedding size (activity + resource)
        ...
        self.dim_init_prefix = self.activity_emb_size + self.resource_emb_size + 2
        ...
        # Dimensionality of initial decoder suffix event tokens after the suffix categoricals are fed to the dedicated entity embeddings and everything, 
        # including the numericals are concatenated
        self.dim_init_suffix = self.activity_emb_size + 2 + self.resource_emb_size
        ...
        # Initializing the additional resource output layer
        self.fc_out_res = nn.Linear(self.d_model, self.num_resources)
    ...
    def forward(self, 
        ...
        """
        # Tensor containing the numerical features of the prefix events. 
        num_ftrs_pref = inputs[2] # (batch_size, window_size, N)

        # Tensor containing the padding mask for the prefix events. 
        padding_mask_input = inputs[3] # (batch_size, window_size) = (B, W)

        # Tensor containing the numerical features of the suffix event tokens: 
        num_ftrs_suf = inputs[6] # (batch_size, window_size, 2)

        # # Constructing categorical embeddings prefix (encoder)
        act_emb_pref = self.act_emb(inputs[0])
        res_emb_pref = self.res_emb(inputs[1])
        cat_emb_pref = torch.cat((act_emb_pref, res_emb_pref), dim=-1)
        ...
        if self.training: # Teacher forcing (for now)
            ...
            # Using the activity embedding layer shared with the encoder
            act_emb_suf = self.act_emb(inputs[4])
            res_emb_suf = self.res_emb(inputs[5])
            cat_emb_suf = torch.cat((act_emb_suf, res_emb_suf), dim=-1)
            #cat_emb_suf = self.act_emb(inputs[0]) # (batch_size, window_size, embed_sz_categ[0])
            ...
            # Next resource prediction head:
            res_probs = self.fc_out_res(dec_output)  # (batch_size, window_size, self.num_resources)
            ...
            if self.only_rrt:
                ...
                return act_probs, ttne_pred, rrt_pred , res_probs
                # (batch_size, window_size, self.num_activities), (batch_size, window_size, 1), (batch_size, window_size, 1)
            elif self.only_out:
                out_pred = self.fc_out_out(dec_output) # (batch_size, window_size, 1)
                out_pred = self.sigmoid_out(out_pred) # (batch_size, window_size, 1)
                # Only first decoding step output needed 
                out_pred = out_pred[:, 0, :] # (batch_size, 1)
                return act_probs, ttne_pred, out_pred, res_probs
            elif self.both:
                rrt_pred = self.fc_out_rrt(dec_output) # (batch_size, window_size, 1)
                ...
                # Only first decoding step output needed 
                out_pred = out_pred[:, 0, :] # (batch_size, 1)
                return act_probs, ttne_pred, rrt_pred, out_pred, res_probs
            else: 
                return act_probs, ttne_pred, res_probs
                # (batch_size, window_size, self.num_activities), (batch_size, wi5ndow_size, 1)
            ...
            act_inputs = inputs[4] # (B, W)
            res_inputs = inputs[5] # verify this too

            batch_size = act_inputs.size(0) # B
            batch_size_res = res_inputs.size(0)  # B

            # Initializing zero filled tensors for storing the activity 
            # and timestamp predictions during decoding 
            suffix_acts_decoded = torch.full(size=(batch_size, window_size), fill_value=0, dtype=torch.int64).to(device) # (B, W)
            suffix_ttne_preds = torch.full(size=(batch_size, window_size), fill_value=0, dtype=torch.float32).to(device) # (B, W)
            suffix_res_decoded = torch.full(size=(batch_size, window_size), fill_value=0, dtype=torch.int64).to(
                device)  # (B, W)

            for dec_step in range(0, window_size):
                # Leveraging learned embedding 
                emb_suf_act = self.act_emb(act_inputs) # (B, W, self.activity_emb_size)
                emb_suf_res = self.res_emb(res_inputs)
                cat_emb_suf = torch.cat((emb_suf_act, emb_suf_res), dim=-1)
                # Concatenating both
                target_in = torch.cat((cat_emb_suf, num_ftrs_suf), dim = -1) # (B, W, dim_init_suffix)
                ...
                # Next resource prediction head:
                res_logits = self.fc_out_res(dec_output)  # (B, W, self.num_resources)
                ...
                #   Selecting predictions for current decoding step
                act_outputs = act_logits[:, dec_step, :] # (B, C)
                res_outputs = res_logits[:, dec_step, :]  # (B, C)
                ttne_outputs = ttne_pred[:, dec_step, 0] # (B, )
                ...
                # Decoding resource preditions (greedily)
                res_outputs[:, 0] = -1e9
                #   Greedy selection
                res_selected = torch.argmax(res_outputs, dim=-1)  # (batch_size,), torch.int64
                #   Adding selected resource integers to suffix_acts_decoded
                suffix_res_decoded[:, dec_step] = res_selected
                ...
                if dec_step < (window_size-1):
                    ...
                    # step. 
                    act_suf_updates = act_selected.clone() # (batch_size, )
                    res_suf_updates = res_selected.clone()  # (batch_size, )
                    #   There is no artificially added END token present in the 
                    #   suffix activity representations, and hence there is no 
                    #   end token index in the suffix activity representations 
                    #   on index num_activities-1. Therefore, we clamp 
                    ...
                    #   instances will not be taken into account at the end. 
                    act_suf_updates = torch.clamp(act_suf_updates, max=self.num_activities-2) # (batch_size,) aka (B,)
                    res_suf_updates = torch.clamp(res_suf_updates,
                                                  max=self.num_resources - 2)  # (batch_size,) aka (B,)
                    # Updating `act_inputs` for suffix decoder for next decoding step 

                    act_inputs[:, dec_step+1] = act_suf_updates # (B, W)
                    res_inputs[:, dec_step + 1] = res_suf_updates  # (B, W)
                    # Deriving TSS and TSP time features for next decoding 
                    # step based on the TTNE predictions 
            ...
            if self.only_rrt:
                ...
                return suffix_acts_decoded, suffix_ttne_preds, rrt_pred, suffix_res_decoded
                # (B, W), (B, W) and (B,)
            elif self.only_out:
                return suffix_acts_decoded, suffix_ttne_preds, out_pred, suffix_res_decoded
                # (B, W), (B, W) and (B, )
            elif self.both:
                return suffix_acts_decoded, suffix_ttne_preds, rrt_pred, out_pred, suffix_res_decoded
                # (B, W), (B, W), (B,) and (B, )
            else:
                return suffix_acts_decoded, suffix_ttne_preds, suffix_res_decoded
                # (B, W), (B, W)