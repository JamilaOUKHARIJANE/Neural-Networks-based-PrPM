...
def aggregate_results(log_name, alg,models_folder,beam_size=3, resource=False,timestamp=False,outcome=False,probability_reduction=False):
    average_act=[]
    average_res =[]
    for fold in range(shared.folds):
        ...
        df_results = pd.read_csv(os.path.join(folder_path, filename), encoding='ISO-8859-1')
...
if __name__ == "__main__":
    ...
    with (open(os.path.join(shared.output_folder, f"aggregated_results_performance.csv"), mode='w') as out_file):
        ...
        writer.writerow(["", "", "", "", "", "with probability reduction", "","without probability reduction", "", "with probability reduction", "","without probability reduction", ""])
        writer.writerow(["Dataset", "Model", "Encoding", "Activities", "Resources", "Activities", "Resources", "Activities", "Resources", "Activities", "Resources", "Activities", "Resources"])
        for log in shared.log_list:
            for models_folder in ["keras_trans", "LSTM"]:
                for encoder in ["_One_hot", "_Simple_categorical", "_Combined_Act_res"]:
                    average_act_baseline, average_res_baseline = aggregate_results(log, 'baseline',models_folder + encoder,resource=True,timestamp=False,outcome=False,probability_reduction=False)
                    average_act_beamsearch3_withPRed, average_res_beamsearch3_withPRed = aggregate_results(log, 'beamsearch', models_folder + encoder, 3, resource=True, timestamp=False, outcome=False,probability_reduction=True)
                    average_act_beamsearch5_withPRed, average_res_beamsearch5_withPRed = aggregate_results(log, 'beamsearch',
                                                                                       models_folder + encoder, 5,
                                                                                       resource=True, timestamp=False,
                                                                                       outcome=False,probability_reduction=True)
                    average_act_beamsearch3, average_res_beamsearch3 = aggregate_results(log, 'beamsearch',
                                                                                         models_folder + encoder, 3,
                                                                                         resource=True, timestamp=False,
                                                                                         outcome=False, probability_reduction=False)
                    ...
                    writer.writerow([log.removesuffix(".csv"), models_folder, encoder.removeprefix("_"),average_act_baseline,
                                 average_res_baseline, average_act_beamsearch3_withPRed,average_res_beamsearch3_withPRed, average_act_beamsearch3,
                                     average_res_beamsearch3, average_act_beamsearch5_withPRed,average_res_beamsearch5_withPRed,average_act_beamsearch5,average_res_beamsearch5])